...
| `coalesceFindRequests` (batched findRecord calls) | ✅ |
| `IndexedDBCache` (offline-first persistent cache) | ✅ |
| MDQL query builder (`store.select()`) | ✅ |
...
const posts = await store.select<Post>('post')
...
const users = await store.select<User>('user')
  .or(b => {
...
const adults = store.select<User>('user')
...
See the full [MDQL guide](docs/guide/mdql.md) for operators, validation rules, and reactive queries.
...
```ts
import { IndexedDBCache } from 'mobx-data/cache';
...
See the full [cache guide](docs/guide/cache.md) for details.
...
src/
  ...
  schema/           — SchemaService, decorators (@attr, @belongsTo, @hasMany)
...
tests/
  ...
  coverage/         — edge-case tests for 100% coverage
