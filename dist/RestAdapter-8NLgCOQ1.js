...
const y = new RegExp("([\\p{Ll}\\d])(\\p{Lu})", "gu"), F = new RegExp("(\\p{Lu})([\\p{Lu}][\\p{Ll}])", "gu"), L = new RegExp("(\\d)\\p{Ll}|(\\p{L})\\d", "u"), U = /[^\p{L}\d]+/giu, i = "$1\0$2", l = "";
function d(c) {
  let e = c.trim();
  e = e.replace(y, i).replace(F, i), e = e.replace(U, "\0");
  let r = 0, t = e.length;
  for (; e.charAt(r) === "\0"; )
    r++;
  if (r === t)
...
function x(c, e) {
  const [r, t, s] = f(c, e), n = h(e == null ? void 0 : e.locale), a = m(e == null ? void 0 : e.locale), o = E(n, a);
  return r + t.map(o).join("") + s;
}
function S(c, e) {
...
function f(c, e = {}) {
  const r = e.split ?? (e.separateNumbers ? b : d), t = e.prefixCharacters ?? l, s = e.suffixCharacters ?? l;
  let n = 0, a = c.length;
  for (; n < c.length; ) {
    const o = c.charAt(n);
    if (!t.includes(o))
...
class g {
  ...
  urlForFindRecord(e, r, t) {
    return this._composeURL(`${this.pathForType(r)}/${e}`);
  }
...
var A = Object.getOwnPropertyDescriptor, O = (c, e, r, t) => {
  for (var s = t > 1 ? void 0 : t ? A(e, r) : e, n = c.length - 1, a; n >= 0; n--)
    (a = c[n]) && (s = a(s) || s);
  return s;
};
let u = class extends g {
  ...
  async findMany(c, e, r, t) {
    const s = this.buildURL(e, r, t, "findMany"), n = s.includes("?") ? "&" : "?", a = `${s}${n}ids=${r.map(encodeURIComponent).join(",")}`;
...
//# sourceMappingURL=RestAdapter-8NLgCOQ1.js.map
